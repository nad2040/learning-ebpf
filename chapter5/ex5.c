...
int main()
{
	...
	int keys[2] = {0, 501};
	char values[2][12] = {"Hey root!", "Hi user 501!"};
	for (int i = 0; i < 2; ++i) {
		err = bpf_map__update_elem(skel->maps.my_config, &keys[i], sizeof(keys[i]), values[i], 12, 0);
		if (err) {
			fprintf(stderr, "Failed to add to map my_config: '%s'\n", values[i]);
			hello_buffer_config_bpf__destroy(skel);
			return 1;
		}
	}
